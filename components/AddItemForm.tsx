...
export const AddItemForm: React.FC<AddItemFormProps> = ({ onAddItem, onClose }) => {
  ...
  const [category, setCategory] = useState<ClothingCategory>('upper');
  ...
  const handleSubmit = useCallback((e: React.FormEvent) => {
    ...
    onClose();
  }, [name, category, image, onAddItem, onClose]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-4 bg-surface rounded-lg">
      ...
      <div>
        ...
        <select value={category} onChange={(e) => setCategory(e.target.value as ClothingCategory)} className="mt-1 block w-full p-2 border border-gray-600 rounded-md bg-background text-text-primary focus:ring-primary focus:border-primary">
